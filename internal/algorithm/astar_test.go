...
func TestAStar_ValidPath(t *testing.T) {
	...
	result, err := AStar(grid, start, goal)
...
func TestAStar_OutOfBounds(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := AStar(grid, tt.start, tt.goal)
...
func TestAStar_Blocked(t *testing.T) {
	...
	_, err = AStar(grid2, start2, goal2)
	assert.ErrorIs(t, err, ErrBlocked)
}
...
func TestAStar_LinearPath(t *testing.T) {
...
func TestAStar_OptimalPath(t *testing.T) {
	// A* should find optimal paths
	grid := createTestGrid(5, 5, []maze.Point{
		{X: 2, Y: 0},
		{X: 2, Y: 1},
		{X: 2, Y: 3},
		{X: 2, Y: 4},
	})
	start := maze.Point{X: 0, Y: 2}
	goal := maze.Point{X: 4, Y: 2}

	result, err := AStar(grid, start, goal)
	require.NoError(t, err)
	assert.True(t, result.Found)
	// Optimal path should be through the gap at Y=2
	// Should be shortest path possible
	assert.LessOrEqual(t, result.PathLength, 5)
}

func TestAStar_ExpandedNodes(t *testing.T) {
	...
	require.NoError(t, err)
	assert.True(t, result.Found)
	// A* should expand fewer nodes than BFS/DFS due to heuristic
	assert.Greater(t, result.ExpandedNodes, 0)
}
